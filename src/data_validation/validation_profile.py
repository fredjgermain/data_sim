...
class ValidationProfile(Profile):
    ...
    @classmethod 
    def validate(cls, data:pd.DataFrame) -> ProfileValidationReport: 
      report = ProfileValidationReport()
      for name, fld in cls.inspect().items(): 
        for ann in fld.get_many(IValid): 
          result = ann.validate(ValidCtx(name, data)) # tuple[float, Series[Any], bool]
          report.update(fld, ann, result) 
      return report